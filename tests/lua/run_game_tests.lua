...
local function serializable_while_loop()
    local state=ready_state()
    local game=Game.new(catalog("while_random.xml"),state,function() return 6 end)
    assert(game:load("1","test")); assert(game:choose(action_index(game,"random")))
    equal(state.variables.done,nil,"while remains active after sentinel roll")
    equal(state.execution.frames[1].kind,"while","serialized while frame")
    equal(state.execution.frames[1].iteration,2,"serialized while iteration")
    equal(game.actions[1].kind,"random","next while iteration blocks")
    local saved=State.copy(state)
    local restored=Game.new(catalog("while_random.xml"),saved,function() return 1 end)
    assert(restored:load("1","test")); equal(restored.actions[1].kind,"random","restored while blocker")
    assert(restored:choose(action_index(restored,"random")))
    equal(saved.variables.done,1,"restored while termination")
    equal(saved.variables.after,1,"execution continues after while")
